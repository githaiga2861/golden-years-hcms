...
function ClientModal({ client, onClose, onSaved }) {
  ...
  const [existingCarePlanFile, setExistingCarePlanFile] = useState(null)
  ...
  useEffect(() => {
    ...
    if (!isNew) {
      ...
      supabase.from('client_documents').select('*').eq('client_id', client.id).eq('doc_type', 'care_plan')
        .order('created_at', { ascending: false }).limit(1).maybeSingle()
        .then(({ data }) => setExistingCarePlanFile(data))
    }
  ...
  const save = async () => {
    ...
    try {
      ...
      if (carePlanFile) {
        const path = `${cid}/${Date.now()}_${carePlanFile.name}`
        const { error: upErr } = await supabase.storage.from('client-documents').upload(path, carePlanFile)
        if (upErr) throw new Error(`Care plan file: ${upErr.message}`)
        const { data: { user } } = await supabase.auth.getUser()
        await supabase.from('client_documents').insert({
          client_id: cid, doc_type: 'care_plan', title: carePlanFile.name, storage_path: path, uploaded_by: user.id,
        })
      }
    ...
    <Modal title={isNew ? 'Register client' : 'Edit client'} onClose={onClose} wide footer={
      ...
      {step === 'clinical' && (
        <>
          ...
          <h3 className="thread mt">Care plan file (optional)</h3>
          <p className="muted" style={{ fontSize: '.84rem' }}>
            Upload the physician-signed care plan (PDF or Word doc). This is for office reference only —
            it is never shown to caregivers.
          </p>
          {existingCarePlanFile && !carePlanFile && (
            <p className="notice notice-ok" style={{ fontSize: '.86rem' }}>
              Current file: <b>{existingCarePlanFile.title}</b> (uploaded {new Date(existingCarePlanFile.created_at).toLocaleDateString()}).
              Choose a new file below to replace it.
            </p>
          )}
          <input type="file" accept=".pdf,.doc,.docx" onChange={(e) => setCarePlanFile(e.target.files?.[0] || null)} />
